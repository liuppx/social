...
import com.bx.imcommon.mq.api.MessageQueueTemplate;
 ...
 * @version: 1.0
 */
public class RedisMQTemplate extends RedisTemplate<String, Object> implements MessageQueueTemplate {
    ...
    public Long enqueue(String queue, Object payload) {
        return opsForList().rightPush(queue, payload);
    }

    @Override
    public Object dequeue(String queue) {
        return opsForList().leftPop(queue);
    }

    @Override
    public List<Object> dequeue(String queue, int batchSize) {
        return opsForList().leftPop(queue, batchSize);
    }
