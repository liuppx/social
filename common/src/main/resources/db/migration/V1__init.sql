create table `t_user` (
  ...
  `did` varchar(255) default '' comment 'DID标识',
  `wallet_address` varchar(64) default '' comment '主钱包地址',
  `wallet_type` varchar(32) default '' comment '钱包链类型',
  `wallet_verified_at` datetime default null comment '钱包绑定时间',
...
create table `t_wallet` (
  ...
  `public_key` varchar(130) default '' comment '公钥',
  `is_primary` tinyint(1) default 0 comment '是否主钱包',
  `created_time` datetime default current_timestamp comment '创建时间',
  unique key `uk_wallet_address` (`address`,`chain_type`),
  key `idx_wallet_user_id` (`user_id`)
) engine = innodb charset = utf8mb4 comment '钱包绑定';
