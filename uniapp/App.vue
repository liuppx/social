...
.tab-page {
	...
	// #ifdef H5
	height: calc(100vh - 50px - $yeying-nav-bar-height); // h5平台100vh是包含了底部高度，需要减去
	top: $yeying-nav-bar-height;
	// #endif

	// #ifdef APP-PLUS
	height: calc(100vh - var(--status-bar-height) - $yeying-nav-bar-height); // app平台还要减去顶部手机状态栏高度
	top: calc($yeying-nav-bar-height + var(--status-bar-height));
	// #endif

	// #ifdef MP-WEIXIN
	height: calc(100vh - $yeying-nav-bar-height);
	top: $yeying-nav-bar-height;
...
.page {
	position: relative;
	// #ifdef H5
	height: calc(100vh - $yeying-nav-bar-height); // h5平台100vh是包含了底部高度，需要减去
	top: $yeying-nav-bar-height;
	// #endif
